...
void main (void)
{
  ...
  ivec2 size = imageSize(colour);
    ...
    vec4 dst = vec4(tnear, D, 0.0, inbox ? 0.0 : 1.0);
    dst.a = D > StepSize ? dst.a : 1.0;
    imageStore(dataIn, storePos, dst);
    imageStore(colour, storePos, vec4(vec3(0.0),0.0));
    //dst.r = size.x > 1.0 ? 1.0 : 0.0;
    //dst.g = size.y > 1.0 ? 1.0 : 0.0;
    //dst.r = !inbox ? 1.0 : 0.0;
    //FragColor = dst;//vec4(1.0,1.0,1.0,1.0);
}